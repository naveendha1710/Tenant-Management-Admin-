...
CREATE TABLE IF NOT EXISTS public.form_categories (
  ...
  CONSTRAINT form_categories_unique UNIQUE (form_type, short_code)
);
...
CREATE TABLE IF NOT EXISTS public.form_subcategories (
  ...
  CONSTRAINT form_subcategories_category_fkey FOREIGN KEY (category_id) REFERENCES public.form_categories(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS public.form_options (
  ...
  CONSTRAINT form_options_category_fkey FOREIGN KEY (category_id) REFERENCES public.form_categories(id) ON DELETE CASCADE
);

CREATE INDEX IF NOT EXISTS idx_form_categories_type ON public.form_categories(form_type);
...
CREATE TRIGGER trigger_form_categories_updated_at
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      INSERT INTO form_categories (form_type, name, short_code)
      VALUES (
        'asset',
