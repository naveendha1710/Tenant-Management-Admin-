...
export default function AssetMovement() {
  ...
  useEffect(() => {
    ...
    // For Location: Load based on tenant selection
  ...
  }, [formData.tenant_id, formData.from_tenant_type, formData.movement_type, activeTab, assetListPage, searchTerm]);
  ...
  const loadAssetsPaginated = async (page: number, search: string = '') => {
    try {
      ...
      console.error('Failed to load paginated assets:', error);
    } finally {
