...
export default function AssetMovement() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // If approval is required, start the workflow. If it fails, roll back the movement.
      if (formData.approval_required) {
        try {
          const isSystemUser = user?.appUser?.role !== 'Tenant';
          await workflowEngine.startWorkflow(
            'asset_movement',
            movement.id,
            isSystemUser ? undefined : (formData.tenant_id || undefined),
            {
              request_number: requestNumber,
                  ...
                  <Card className="border-gray-200">
                    ...
                    <CardContent>
                      <div className="space-y-4">
                        ...
                        {formData.from_tenant_type === 'Other' ? (
                          <div className="space-y-4">
                            <div>
                        ...
                        <div>
                          ...
                          <Select value={formData.from_building} onValueChange={(v) => updateField('from_building', v)} disabled={formData.from_tenant_type === 'Tenant' && !formData.tenant_id}>
